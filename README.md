...
cw doctor
cw recordings list
cw recordings latest --output ./output/downloads/recordings/latest
cw announcements list
cw contents tree "音乐与数学"
cw assignments list
```
...
cw contents list
cw announcements show "课程群二维码"
cw assignments submit "L4作业提交入口" --text "draft body" --save-draft
```

Once `cw use <course>` has been run, most course-aware commands can omit the course argument.

## Credentials discovery

`courseweb auth login` looks for credentials in this order:
...
```bash
COURSEWEB_HOME=/custom/path courseweb auth status
```
